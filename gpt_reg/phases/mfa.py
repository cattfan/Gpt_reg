...
from gpt_reg.totp_helper import generate_code, normalize_secret
...
_BACKOFF_CF_SECONDS = (8.0, 18.0, 40.0)
...
async def enable_2fa(
    *,
    ...
    require_enroll_persistence: bool = False,
    log=print,
) -> dict[str, Any]:
    ...
    Args:
        ...
        fingerprint_profile: Tên profile canonical dùng chung cho TLS và headers.
        ...
        require_enroll_persistence: Khi True, enrollment mới chỉ được activate
            sau khi ``on_enroll`` hoàn tất thành công. Thiếu callback hoặc callback
            raise sẽ dừng trước activate với ``MfaError.partial_state``. Mặc định
            False để giữ tương thích với các flow legacy.
        log: callable.
    ...
    async with AsyncSession(
        ...
        if pending_enrollment and pending_enrollment.get("secret"):
            ...
            if on_enroll is not None:
                try:
                    await on_enroll(enrolled_state)
                except Exception as exc_persist:
                    if require_enroll_persistence:
                        log("[mfa] enrollment persistence failed; activation blocked")
                        raise MfaError(
                            "MFA enrollment could not be durably persisted",
                            partial_state=enrolled_state,
                        ) from exc_persist
                    # Best-effort legacy policy: keep activating for callers that
                    # have not opted into durable enrollment checkpoints.
                    log(f"[mfa] WARN on_enroll callback raised: {exc_persist}")
            elif require_enroll_persistence:
                log("[mfa] enrollment persistence callback missing; activation blocked")
                raise MfaError(
                    "MFA enrollment persistence is required before activation",
                    partial_state=enrolled_state,
                )
