...
from gpt_reg.fingerprint import (
    Profile,
    get_profile,
    navigator_payload as _profile_navigator_payload,
    validate_seed,
)
...
def _profile_headers(session) -> dict:
    """Không còn tự dựng header nhận dạng — curl_cffi đã gửi đúng theo
    `impersonate` (giá trị lẫn thứ tự). Giữ hàm này trả rỗng để call site chỉ
    thêm header theo ngữ cảnh."""
    return {}


def _navigator_payload_for(session) -> dict:
    """Navigator persona khớp profile đang gắn trên session.

    sdk.js chạy trong Node thấy `navigator.userAgent` từ payload này. Nếu nó
    khác UA thật gửi trong header thì fingerprint mâu thuẫn và deep validation
    của OpenAI trượt — đúng lỗi từng làm OTP bị silent-drop.
    """
    return _profile_navigator_payload(_profile_of(session), _seed_of(session))
...
def _ensure_sdk_file(session: Any, timeout_ms: int) -> Path:
    ...
    headers = _profile_headers(session)
    headers.update({
        "accept": "*/*",
...
class SentinelNodeWorker:
        ...
        *,
        ...
        with self._lock:
            ...
            while True:
                ...
                if remaining <= 0:
                    # Deadline phải thắng cả khi Node còn sống mà không trả lời.
                    # sdk.js chạy dưới eval với setTimeout bị override thành đồng
                    # bộ; một vòng lặp CPU-bound làm chết event loop Node, timer
                    # nội bộ của worker cũng không chạy được, nên readline() sẽ
                    # block vĩnh viễn nếu ta tin vào nó. Giết process để lần sau
                    # Respawn worker rồi báo lỗi chính xác cho caller.
                    self._kill_proc_locked()
                    raise RuntimeError(f"sentinel worker timeout (action={action})")

                out = self._readline_with_timeout(proc, remaining)
                if out is None:
                    continue  # hết lát chờ — vòng sau kiểm tra deadline
                if not out:
    ...
    def _readline_with_timeout(self, proc, timeout_s: float) -> str | None:
        """Đọc 1 dòng, tối đa `timeout_s`. None = hết giờ, "" = EOF.

        `readline()` của pipe không có timeout, nên đọc trong thread phụ và join
        có hạn. Thread nền có thể còn sống sau khi hết giờ — không sao vì ta giết
        luôn process ngay sau đó, readline sẽ trả EOF và thread tự kết thúc.
        """
        box: list[str] = []
        ...
        reader.start()
        reader.join(timeout=max(0.05, timeout_s))
        if reader.is_alive():
            return None
        return box[0] if box else ""

    def _kill_proc_locked(self) -> None:
        """Giết process đang treo. Chỉ gọi khi đã giữ self._lock."""
        proc = self._proc
        self._proc = None
        if proc is None:
            return
...
def get_sentinel_token_via_quickjs(
    ...
    *,
    ...
    """Run the QuickJS Sentinel path or raise the exact failure.
    ...
    nav_payload = _navigator_payload_for(session)
    ...
    if not t_value:
        raise RuntimeError("QuickJS solve did not return valid t")
