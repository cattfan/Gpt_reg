...
function installRuntime(payload) {
  ...
  const navigatorValue = (function () {
    ...
    const nav = {
      ...
      hardwareConcurrency: Number(payload.hardware_concurrency || 1),
      platform: String(payload.platform || ""),
      vendor: String(payload.vendor || ""),
      webdriver: false,
    };
    if (payload.device_memory !== null && payload.device_memory !== undefined) {
      nav.deviceMemory = Number(payload.device_memory);
    }
    if (payload.has_user_agent_data === true) {
      const brands = Array.isArray(payload.sec_ch_ua_brands)
        ? payload.sec_ch_ua_brands
        : [];
      if (!brands.length) throw new Error("missing Client Hint brands");
      const platform = String(payload.sec_ch_ua_platform || "");
      ...
      const primary = brands.find(b => !/not/i.test(String(b.brand || ""))) || brands[0];
      const fullVersion = String(primary.version || "0") + ".0.0.0";
      nav.userAgentData = {
